...
def search_semantic_scholar_by_venue(query: str, max_results):
    ...
    keywords_raw = os.getenv("KEYWORDS_SEMATIC_MPA")
    try:
...
def save_all_papers_to_csv(papers):
    ...
    keywords_raw = os.getenv("KEYWORDS_SEMATIC_MPA", "results").replace(",", "_").replace(" ", "_")
    output_csv = base_path / f"{keywords_raw}_papers_sematic.csv"
...
if __name__ == "__main__":
    load_dotenv()
    keyword = os.getenv("KEYWORDS_SEMATIC_MPA")
